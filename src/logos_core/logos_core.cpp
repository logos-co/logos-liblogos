...
{
    if (!plugin_name) {
        fprintf(stderr, "logos_core_load_plugin: plugin_name must not be null\n");
        std::abort();
    }
...
{
    if (!plugin_name) {
        fprintf(stderr, "logos_core_load_plugin_with_dependencies: plugin_name must not be null\n");
        std::abort();
    }
...
{
    if (!plugin_name) {
        fprintf(stderr, "logos_core_unload_plugin: plugin_name must not be null\n");
        std::abort();
    }
...
{
    if (!plugin_path) {
        fprintf(stderr, "logos_core_process_plugin: plugin_path must not be null\n");
        std::abort();
    }
...
{
    if (!key) {
        fprintf(stderr, "logos_core_get_token: key must not be null\n");
        std::abort();
    }
