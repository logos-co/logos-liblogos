...
// already saved when installed, so an embedder that calls modules through its
// own token store can mirror them there. Calls are serialized and must not
// call back into core; once NULL is installed, none is running or will run.
