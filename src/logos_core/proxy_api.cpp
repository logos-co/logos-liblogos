...
{
    if (!callback) {
        qWarning() << "asyncOperation: callback is null";
        return;
    }
...
{
    if (!callback) {
        qWarning() << "loadPluginAsync: callback is null";
        return;
    }
...
void callPluginMethodAsync(
    ...
    if (!callback) {
        qWarning() << "callPluginMethodAsync: callback is null";
        return;
    }
...
void registerEventListener(
    ...
    if (!plugin_name || !event_name || !callback) {
        qWarning() << "registerEventListener: Invalid parameters";
        return;
    }
