...
- **Additional containers** — Register alternative `ModuleContainer` implementations (e.g. Docker, in-process, sandboxed) that can be composed with any loader. The container contract lives in its own [`logos-container`](https://github.com/logos-co/logos-container) package and the subprocess implementation in [`logos-container-subprocess`](https://github.com/logos-co/logos-container-subprocess), so a new container is a standalone package implementing the same interface — no change to `liblogos_core`. **This already works additively today:** a frontend (or any embedder) can register its own loader/container via `ModuleManager::loaders().registerLoader(...)` before `logos_core_start()`, and a module can pin one by id with `loaderConfig["id"]`. The new loader composes with the built-in subprocess default.
- **Frontend-owned loader registration (decoupling the default)** — The core still self-registers a built-in subprocess + qt-plugin loader, which is the only reason `liblogos_core` build-depends on `logos-container-subprocess`. A future step removes that default so the core depends *only* on the `logos-container` contract and each frontend (logoscore-cli, basecamp) constructs and registers the container(s) it wants at startup. Deferred deliberately — it touches the frontend repos, and the built-in default lets them work with zero startup wiring until a second container exists. The migration path is documented at the `loaderRegistry()` construction site in `module_manager.cpp`.
