...
TEST_F(AppLifecycleTest, Deprecated_SetModulesDir_ClearsExisting) {
    logos_core_add_modules_dir("/dir1");
    logos_core_add_modules_dir("/dir2");
    ASSERT_EQ(logos_core_get_modules_dirs_count(), 2);
...
TEST_F(AppLifecycleTest, Deprecated_Exec_ReturnsZero) {
    logos_core_cleanup();
...
TEST_F(AppLifecycleTest, Deprecated_ProcessEvents_DoesNotCrash) {
    EXPECT_NO_THROW(logos_core_process_events());
}

TEST_F(AppLifecycleTest, Deprecated_ProcessEvents_AfterCleanupDoesNotCrash) {
    logos_core_cleanup();
